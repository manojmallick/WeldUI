...
import { styles } from './wu-social-login.styles.js';
...
/**
 * Social / OAuth login button group.
 *
 * @element wu-social-login
 * @event wu-provider-click - detail: { provider: string }
 */
@customElement('wu-social-login')
export class WuSocialLogin extends LitElement {
  static styles = styles;

  @property({ type: Array }) providers: SocialProvider[] = defaultProviders;
        ...
        ${repeat(
          ...
          (p) => html`
            <button
              ...
              @click=${() =>
                this.dispatchEvent(
                  new CustomEvent('wu-provider-click', {
                    bubbles: true,
                    composed: true,
            ...
            >
              <span class="provider-icon" aria-hidden="true">
                ...
                ` : html`<span style="font-size:12px;font-weight:700">${p.id.slice(0,2).toUpperCase()}</span>`}
              </span>
              <span>${p.label}</span>
            </button>
          `,
