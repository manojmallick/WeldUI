...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Application shell layout that composes `<wu-navbar>` + `<wu-sidebar>` + main content. ' +
          'Use the `navbar` slot for the top bar, `sidebar` slot for side nav, and the default slot ' +
          'for page content. Set `full-width` to remove the max-width constraint on the content area.',
      },
    },
  },
};
...
/** Full application shell — 600px tall preview. */
